		...
		<template v-else>
			<n-form class="is-variant-vertical" content-class="is-column is-spacing-medium is-width-max-xsmall is-width-min-xsmall is-color-background is-border-full process-modeler-form">
				...
				<div v-if="selected.type == 'action'" class="is-column is-spacing-gap-medium">
					...
					<n-form-switch :edit="editable" v-if="selected.target.actionType == 'reset'" v-model="selected.target.resetData" label="Reset all data" after="By default only the process action instances are reset and data is retained. You can unset specific data with value capturing or if you enable this you reset all data that was captured by the process action instances that are will be reset"/>
					...
					<n-form-switch :edit="editable" v-if="selected.target.actionType == 'service'" v-model="selected.target.reprocessable" label="Allow reprocessing" @input="draw" after="Do you want to be able to reprocess this service if it fails? This will capture the input if necessary."/>
					...
					<n-form-switch :edit="editable" v-if="selected.target.actionType == 'reset'" v-model="selected.target.automatic" label="Reset automatically" after="When checked, the reset runs automatically." @input="draw"/>
					<n-form-switch :edit="editable" v-if="selected.target.actionType == 'finalizer'" v-model="selected.target.automatic" label="Send signal" after="When checked, the finalizer runs automatically." @input="draw"/>
					<n-form-switch :edit="editable" v-if="['signal', 'service', 'human', 'finalizer', 'reset'].indexOf(selected.target.actionType) >= 0 && selected.target.automatic" v-model="selected.target.synchronous" label="Execute synchronously"  @input="draw" after="The action can be run asynchronously or synchronously depending on this setting"/>
